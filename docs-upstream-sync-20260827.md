...
- 每一页渲染都登记到 Lotus 签到协调器；已经开始的当前页先完成，随后优先执行等待中的自动或手动签到，签到完成后再继续下一页和最终发送。
...
- 数量上限仍为 200；
- 96 条以上仍保留禁用超分逻辑。
- 默认 28 和 39 等低于 40 的设置保持单图，达到 40 后才按 24 分页。
- 每页渲染仍通过 `runNonSigninTask` 与 Lotus 的自动、手动签到共用优先级协议。
